...
**Deployed:** whatever `CACHE` in `sw.js` says — `mq-v69` at the time of writing (the `win` seam: la ventanilla behind a real counter in every camera, `ch-v4`; the reader's Comment button and sign-in line) — before that `mq-v68` (the `ANIMALS` seam, #38; regression R1/R5/R8) and `mq-v67` (Parts 2a, 2b and 3 of El
