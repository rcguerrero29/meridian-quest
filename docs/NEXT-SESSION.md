...
- **Seven persona proposals** from that run are in `docs/crew/FLIGHT-NOTES.md` iteration 6 with verdicts; **seven mural panels** are in the raw file and not yet painted — the next sitting paints them (return visits for Beto, Rigo, Pili, Tavo, Chema; Cuca's and Don Güero's are seconds too).
