...
| **A whole cast placed from data** | `addChill` `:160`, `syncChill` `:179` (numbers re-checked 2026-09-14; grep the two names, they slid six lines) — **and `look` is write-once per id, `docs/TAGS.md` L23** | A leaving present from twelve colleagues is twelve rows of `{name, look, world, x, y, doc}`. That is already an API |
