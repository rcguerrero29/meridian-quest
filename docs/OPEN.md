...
| ~~**Should a town-only change be forced to bump `ch-v`?**~~ **ANSWERED 2026-09-15: "Yes, guard it."** Built the same hour in `test/bump.js` (the town half) and registered as `docs/BOUNDARY.md` **row 15** — which is also the correction Yaz found: this row used to point at row 5, and row 5 is the *player's* browser and mentions neither the town's version nor his laptop. The objection died on a measurement: of 71 commits touching `changarrito/`, ten left the version standing and all ten changed code the town runs, none docs-only | — | `docs/BOUNDARY.md` row 15; `test/bump.js`; `docs/MINOR.md` §2 |
...
| `docs/BEAUTIFY.md` | What every object renders as, what it should be, and which are correctly flat. **Meridian's own register** — the five lessons in it that are true of ANY world were copied into `docs/NEW-WORLD.md` "How a thing gets drawn here" on 2026-09-17, because a second world's builder reads the template and never this | touching any art |
| **`.claude/skills/stairs/`** | **The pre-flight for a door, a staircase, a floor or a world boundary** — the five questions, the three facts about this engine a person otherwise learns the hard way, and every guard that already exists so nobody writes them twice. Opened 2026-09-17: *"lets just plan a fix skill for stairs and world so it makes 'sense'"* | **adding or moving a portal, a flight, a level or a room** |
| **`.claude/skills/guard/`** | The four ways a guard fools the person who wrote it, each with the run that found it. **Reading a guard is not checking it; planting is** | **writing or trusting any test, check, scan or CI step** |
| **`.claude/skills/how-its-made/`** | Draw a thing by the process that made it. *Variation enters at the step it actually entered, and nowhere earlier* | **drawing, modelling or texturing anything that was made** — and always when several of the same thing appear together |
