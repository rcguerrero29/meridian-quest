...
| Question | Why it is blocking | Where it is written |
|---|---|---|
| **The second world about eating well — fifteen decisions he owes, and one confirmation.** Whose game is it (his, AJ's, both — and does it ship publicly at all)? What is the nicest part of cooking for him? Does a meal plan survive A3's ban on a record of the future? Which kitchen first? Is "every business is Hispanic-owned" a Meridian rule or a global one? May a self-help pack grade the kitchen and never the eater? Plus eight more, each with the agent who asked it | **Nothing is built and no mock is drawn until the first two are answered** — four different games sit behind "what is the nicest part of cooking for you", and they look identical in a one-line pitch. The research is done and the plan is written; the decisions are the whole remaining blocker | `docs/la-sobremesa.md` **§9** (the plan; the evidence is `docs/research/2026-09-13-healthy-eating-game.md`, the rules are `docs/GENRE-RULES.md` R15–R17) |
