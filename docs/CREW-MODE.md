...
| **Claiming** | nothing — the session just works | a `taken: <agent>` label on the issue **before** the branch exists |
...
2. **The label is the lock.** An agent claims by labelling *before* it starts; the label clears on
   ...
   **Since 2026-09-20 the label names WHICH AI, not only which persona** — `taken: claude`,
   `taken: codex` — because the owner asked to run a second AI on this engine and *"things labeled
   as in flight on our backlog"* has to answer *whose* flight. The word stays `taken:` because that
   is what the town renders (`record.js`, `takenBy`) and what `CLAUDE.md` says; a 2026-09-20 draft
   wrote `in-flight:` here and the town would never have shown it — corrected 2026-09-21. Inside a
   run, a persona's name may follow the agent's: `taken: claude/beto`. **A label and not a lockfile**, and that is the whole reason it works: two agents editing
