...
| # | Thing | Cost | Detail |
|---|---|---|---|
| ~~1~~ | **SHIPPED 2026-09-05 (mq-v65, Part 1).** The verdict card builds with `textContent`; the guarantee test fails any `innerHTML` interpolation in `engine/` | minutes | `engine.js:2359` builds `concept / why / beat` with innerHTML. Trusted pack text today; the only DOM sink that takes pack text unescaped. Rebuild with `textContent` |
| ~~2~~ | **SHIPPED 2026-09-05 (mq-v65, Part 1).** `res.ok` before the put; own origin only; `index.html` only for navigations; the smoke suite asserts all three on the source | minutes | `sw.js:25-28`. A 403 becomes the permanent answer for that URL. `sw.js` has no runtime test (it cannot register from `file://`) — test its source |
| ~~3~~ | **SHIPPED 2026-09-05 (mq-v65, Part 1)** — `SK()` + `STOREPFX`, 51 sites, default `"mq"` so no key changed; the guarantee test forbids a literal. ~~Twenty literal storage keys in the engine~~ (`"mq1"`, `mqlang`, `mqadmin`, `mqroom`, `mqnpcs`, `mqcam`, `mqtheme`, `mqpark`…) | half a sitting | any second pack on the same origin loads and then overwrites Meridian's save. A pack-declared `STOREPFX`, default `"mq"`, and a smoke assertion that no literal remains. Blocks every second world, AJ's included |
...
| ~~5~~ | **SHIPPED 2026-09-05 (Part 1).** `permissions: contents: read`; `playwright@1.63.0` | minutes | a bad dependency in CI with a write `GITHUB_TOKEN` can push to `main`, which Pages serves. `permissions: contents: read`; pin the version |
| 6 | **`PEERS` looks are not validated** — the comment now says so (Part 1); validation lands with whatever fills `PEERS` | minutes | `p.look` is passed raw to `drawPerson` at 958, 1048 and `engine3d.js:344`. Dormant until `NET` fills it; the comment lies now |
| ~~7~~ | **SHIPPED 2026-09-05 (mq-v65, Part 1).** Prototype keys skipped in the loader | one line | `engine.js:3325-3330` — a crafted `#save=` link can set `__proto__` on a local object. Contained; still one `if` |
| 8 | **`auditReach()` runs once at boot, not after a runtime spawn** — the record re-audits after every placement (2a); the engine still does not | small | `engine.js:174`. `addChill()`/`spawnCustom()` can wall the hero after boot with no check |
| ~~9~~ | **SHIPPED 2026-09-05 (mq-v65, Part 1).** The guarantee block in `test/smoke.js` — seven assertions on the source and one on a loaded page with every flag a reader might guess | half a sitting | nothing asserts that `index.html`, `engine/`, `content/` contain no `api.github.com`, no `location.search`, and that `?dev=1&admin=1` leaves `admin === false`. `el-changarrito.md` §5 R7 lists the assertions |
| 10 | **A wandering neighbour is a wall, and nothing re-audits after a wander step** — found 2026-09-13 by Yaz and Lupe, open | small | Row 8 is the *spawn* half of this; this is the *wander* half, which is every second of play. A person is stamped into the grid as `"N"` on every wander step (`engine/engine.js`, grep `if(w.grid[n.y])w.grid[n.y][n.x]="N"`), and both `isSolid` (grep `const isSolid=`) and `auditReach`'s own `walk` (grep `const walk=(id,x,y)=>`) refuse an `"N"` tile. The wander filter asks about solidity, doors and tram danger and **never asks whether the step cuts the map in two**. In `ex`, a neighbour in the one-tile gap at (19,1) puts 35 tiles and Doña Meche out of reach **of the player as well as the audit**, and reds `test/smoke.js` about **one run in twenty-five**. Either the gap widens in the map, or the filter asks `auditReach` before it steps. `docs/REGRESSION.md` R11 carries the assertion; `docs/QA-PASS.md` E11 carries the escape |
