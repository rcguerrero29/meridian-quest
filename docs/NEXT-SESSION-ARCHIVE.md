# Next session — the archive

> **This is history, not instructions.** The live state of play is `docs/NEXT-SESSION.md`; nothing
> here has been re-checked against the code since the day it was written, and several entries were
> already wrong when they were written. **A fact you find here is a fact about that date.** Newest
> first. Moved out of `docs/NEXT-SESSION.md` on 2026-09-13 verbatim, when that file had reached 924
> lines and only its first hundred were current. Nothing was edited on the way across except one
> bracketed note at `test/cast.js` — the owner's rule is that we do not delete and we capture each
> iteration.
...
an AI (OWNER.md rule); this runs in a session, from the repo, like Pili and Nacho do today.
 ...
 4. *The tests, red first.* (a) `test/cast.js` **— proposed here, never written; as of 2026-09-13 no such file has ever existed in this repo**: every brief builds from content and names a real station, a
