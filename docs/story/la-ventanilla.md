...
| 7 | **Writing it** | `.github/workflows/pages.yml`, `.github/scripts/city-record.js`, `test/record.js` | **BUILT 2026-09-07 (#14, owner: "go for your recommendations" — version and permits, nothing more).** The `Pages` workflow copies the site to `_site/`, the step "write the city record" writes `status.json` from the GitHub API (`v`, `written`, `deployed` = `GAMEV`, `permits[]` = the owner's open PRs with `green` and `mergeable`; a `note` when the permits could not be read), `test/record.js` checks the shape, then `upload-pages-artifact` + `deploy-pages`. CI still never pushes a commit. **Until the owner switches Settings → Pages → Source to "GitHub Actions" the deploy step cannot take (it is allowed to fail) and the branch deploy stands** — the record is checked on every push to `main`, served only after the flip | `city record` |
